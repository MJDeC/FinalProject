...
class DBInterface:
    ...
    def run_inp_loop(self):
        while True:
            ...
            # Create a list holding contents of self.table table (used for input validation).
    ...
    def add_row(self):
        while True:
            ...
            row = [row_id]
            params = ', ?' * (len(self.columns_list) - 1)
            ...
            if inp.lower() == 'exit':
                break
            # Add row to table.
            self.cur.execute(f'''INSERT INTO {self.table} VALUES (?{params})''', row)
            print('Row added successfully.')
            break

    def edit_row(self):

        # Initialize update flag. Once this is true, input loop terminates.
        updated = False
        ...
        # Input loop for getting attribute to be modified. Once valid ID has been taken from user, program asks the user
        # which attribute (or both) in the row they would like to modify.
        while not inp.lower() == 'exit':
            ...
            # Change specific attribute in row.
            else:
...
if __name__ == '__main__':
    ...
    db_inter_1 = DBInterface('info.db', 'Info')
